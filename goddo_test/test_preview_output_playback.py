...
def test_restricted_go_to(app_thread, windows_container: WindowsContainer, blank_state):
...
def test_unrestricted_go_to(app_thread, windows_container: WindowsContainer, blank_state):
    open_clip_on_output_window(app_thread, windows_container, '0:01:00.00', '0:02:00.00', get_blank_1hr_vid_path())
    ...
    assert windows_container.output_window.slider.value() == 0
    assert app_thread.mon.state.preview_window_output.current_frame_no == 4 * 50

    # go to between start frame and in frame
    enter_time_in_go_to_dialog_box(app_thread, '0:00:54.00')
    assert time_edit.styleSheet() == 'background-color: white;'
    assert 9 <= windows_container.output_window.slider.value() <= 10
    assert app_thread.mon.state.preview_window_output.current_frame_no == 4 * 54
    ...
    assert app_thread.mon.state.preview_window_output.current_frame_no == 4 * 126

    # go to end frame
    enter_time_in_go_to_dialog_box(app_thread, '0:02:10.00')
    assert time_edit.styleSheet() == 'background-color: white;'
    assert 199 <= windows_container.output_window.slider.value() <= 200
    assert app_thread.mon.state.preview_window_output.current_frame_no == 4 * 130
...
def test_restricted_move_with_keyboard(app_thread, windows_container: WindowsContainer, blank_state):
    open_clip_on_output_window(app_thread, windows_container, '0:01:00.00', '0:02:00.00', get_blank_1hr_vid_path())
    ...
    pyautogui.press('right')
    wait_until(lambda: app_thread.mon.state.preview_window_output.current_frame_no == 4 * 120)

    pyautogui.press('right')
    time.sleep(0.5)
    assert app_thread.mon.state.preview_window_output.current_frame_no == 4 * 120

    expected_cur_frame = 4 * 60 * 2 - 5
    ...
    generic_assert(app_thread, windows_container, blank_state,
                get_assert_file_list_for_1hr_fn(), get_assert_blank_list_fn(is_file_list=False), 
                get_assert_preview_fn(clip, slider_range=(0.03, 0.04), current_frame_no=expected_out_frame),
                get_assert_preview_fn(clip, slider_range=(0.855, 0.865), current_frame_no=expected_cur_frame, is_output_window=True, extra_frames_left=40, extra_frames_right=40), 
                get_assert_timeline_fn(expected_timeline_clips, selected_clip_index=0, opened_clip_index=0))
...
def test_unrestricted_move_with_mouse_click(app_thread, windows_container: WindowsContainer, blank_state):
    open_clip_on_output_window(app_thread, windows_container, '0:01:00.00', '0:02:00.00', get_blank_1hr_vid_path())
    ...
    click_on_prev_wind_slider(windows_container.output_window, 0.1)
