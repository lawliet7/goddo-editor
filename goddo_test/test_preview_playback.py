...
from goddo_test.utils.test_utils import click_on_prev_wind_slider, drag_and_drop, get_test_vid_path, wait_until, pil_img_to_arr, cmp_image
from goddo_test.utils.windows_container import WindowsContainer

# def test_me(app_thread: QtAppThread, windows_container: WindowsContainer):
#     logging.info('you suck')
#     for attr, value in vars(app_thread.mon.state).items():
#         logging.info(f'var {attr} with type {type(value)} with module {type(value).__module__}')
#     logging.info(app_thread.mon.state.as_dict())

# '''
# iterate all props
#     if basic types put in dict
#     elif videourl then path
# '''
